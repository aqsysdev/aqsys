	...
	User.countUser( function(err, usernum){
	 	...
	 	if(usernum){
			User.ensureAuthenticated(req, res, function(){
				User.getUserById(usernum, function(req,res) {
//					var config = new User.getConfig();
					console.log("User.getUserById:");
					console.log(res.config);
					res.render('register',res.config);
				});
			});
	 	}else{
			res.render('register');
...
router.post('/register', function(req, res){
	...
	req.checkBody('cateString', '1:低学年男,2:低学年女,3:低学年女,4:低学年女,5:中学生男,6:中学生女,7:39才以下男,8:39才以下女,9:40才以上男,A:40才以上女,B:低学年リレー,C:高学年リレー').notEmpty();
